 ...
 * @param result
 */
var TeamWeekSheet2 = function (name) {
    SheetBase.call(this, name);
}

// create prototype from parent class
TeamWeekSheet2.prototype = Object.create(SheetBase.prototype);

// Set the "constructor" property
TeamWeekSheet2.prototype.constructor = TeamWeekSheet2;

TeamWeekSheet2.prototype.Generate = function () {
    ...
    for (var index = 0; index < FFWeekDate.length-1; index++) {
        ...
        for (i = 0 ; i < loopTeam.length; i++) {
            ...
            loopTeam[i].SearchFixedBugFromDateTo(Nightly, undefined, EndDate);
            loopTeam[i].RenderBugNumToSheet(this.sheet, resultRow + rowsTeamResult*i + 1, resultColumn+index);
            loopTeam[i].SearchFixedBugFromDateTo(Beta, undefined, EndDate);
            loopTeam[i].RenderBugNumToSheet(this.sheet, resultRow + rowsTeamResult*i + 2, resultColumn+index);
            loopTeam[i].SearchFixedBugFromDateTo(Release, undefined, EndDate);
...
function RunTeamWeekSheet2() {

    // Generate Sheet
    var Sheet = new TeamWeekSheet2("WeekSheet2");
    Sheet.Generate();
}
